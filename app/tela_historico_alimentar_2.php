...
<body class="bodyA">
    <header class="cabecalho">
        <h1 class="text-xl">Histórico alimentar personalizado do <?=$_SESSION["ref"]?></h1>
        <h1 class="text-little important-text text-bold">Período: <?=$_SESSION["inicial"]?> -- <?=$_SESSION["final"]?></h1>
    </header>
    <div class="box-xl scroll xl">
            <?php 
            
            $datas = $paciente->getPersonalizeDateByRef($_SESSION["inicial"], $_SESSION["final"], $_SESSION["ref"]);
            if (count($datas) > 0){
    ...
    <button class="btn btnAbsolute" onclick="document.location='tela_plano_historico_2.php'">Voltar</button>
</body>
</html>